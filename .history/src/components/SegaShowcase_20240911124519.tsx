...
const SegaShowcase: React.FC = () => {
  ...
  return (
    <div className="w-full py-16 px-4 bg-gradient-to-br from-purple-900 via-pink-800 to-purple-900 relative overflow-hidden">
      <div className="absolute inset-0 bg-pattern opacity-10"></div>
      <div className="max-w-7xl mx-auto relative z-10">
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-center font-bold mb-12 sega-barbie-title text-pink-300 animate-pulse">
          Sega Showcase
        </h2>
        <div className="flex flex-wrap justify-center gap-6 mb-12">
          {games.map((game) => (
            <div
              key={game.id}
              className="w-40 h-40 sm:w-48 sm:h-48 md:w-56 md:h-56 lg:w-64 lg:h-64 bg-black border-4 border-[#FF1493] rounded-lg overflow-hidden cursor-pointer transition-all duration-300 hover:scale-105 relative shadow-neon-pink"
        ...
        {selectedGame && (
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 px-4">
            <div className="bg-[#4B0082] p-6 rounded-lg border-4 border-[#FF1493] max-w-lg w-full shadow-neon-pink">
