  ...
  const handleAccessoryClick = (accessory: Accessory) => {
    ...
    setSelectedAccessories(prev => {
      ...
      if (existingIndex !== -1) {
        ...
        console.log('Adding accessory:', accessory.id);
        return [...prev, { 
          ...accessory, 
          position: { x: 50, y: 50 }, // Start in the middle of the avatar
          scale: 1, 
          rotation: 0 
        }];
      }
        ...
        prev.map(item => {
          if (item.id === activeAccessory) {
            switch (action) {
              ...
              case 'rotate':
                const center = { x: item.position.x + 50, y: item.position.y + 50 };
