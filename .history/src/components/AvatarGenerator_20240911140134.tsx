    ...
    if (avatarRef.current) {
      ...
      try {
        ...
        const canvas = await html2canvas(avatarRef.current, {
          ...
          scale: window.devicePixelRatio, // Используем devicePixelRatio вместо фиксированного значения
          useCORS: true, // Разрешаем загрузку изображений с других доменов
          logging: false, // Отключаем логирование для улучшения производительности
          windowWidth: avatarRef.current.offsetWidth,
          windowHeight: avatarRef.current.offsetHeight,
        });

        // Создаем новый canvas с фиксированным размером
        const fixedCanvas = document.createElement('canvas');
        const ctx = fixedCanvas.getContext('2d');
        fixedCanvas.width = 1200; // Фиксированная ширина
        fixedCanvas.height = 1200; // Фиксированная высота
        ...
        if (ctx) {
          ...
          // Вычисляем размер и позицию для центрирования изображения
          const scale = Math.min(fixedCanvas.width / canvas.width, fixedCanvas.height / canvas.height);
          const x = (fixedCanvas.width / 2) - (canvas.width / 2) * scale;
          const y = (fixedCanvas.height / 2) - (canvas.height / 2) * scale;

          // Отрисовываем изображение по центру
          ctx.drawImage(canvas, x, y, canvas.width * scale, canvas.height * scale);
        }

        const imageDataUrl = fixedCanvas.toDataURL('image/png');
    ...
    if (editingAvatarIndex !== null) {
      ...
      if (imageDataUrl) {
        try {
          ...
          setError(null); // Очищаем ошибк��, если обновление прошло успешно
        } catch (error) {
