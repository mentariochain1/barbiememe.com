...
const Hero: React.FC = () => {
  ...
  const [holders, setHolders] = useState("42,069");
  ...
  useEffect(() => {
    ...
    // Имитация обновления цены и количества держателей
    const priceUpdater = setInterval(() => {
      setPrice((Math.random() * 0.0001).toFixed(6));
      setHolders((Math.floor(Math.random() * 1000) + 42000).toString());
    }, 5000);
  ...
  return (
    <div className="flex flex-col items-center w-full pt-4 sm:pt-8 md:pt-12 px-4">
      ...
      <div className="bg-pink-200 rounded-lg p-4 mb-6">
        <p className="text-pink-800 font-bold">Текущая цена: ${price} | Держателей: {holders}</p>
      </div>
      <div className="mb-6">
