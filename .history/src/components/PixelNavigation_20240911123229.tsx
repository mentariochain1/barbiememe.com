    ...
    <nav className="relative z-50">
      <div className="flex justify-between items-center p-4">
        <button
          ...
          className="md:hidden text-[#FF1493] text-2xl"
        >
          <FaBars />
        </button>
        <a
      ...
      <div className={`${isMenuOpen ? 'block' : 'hidden'} md:block md:flex md:flex-wrap md:justify-center md:gap-2 text-xs sm:text-sm md:text-base font-bold p-2 rounded-lg absolute w-full md:relative`}>
        {menuItems.map(({ name, icon: Icon }) => (
          <a
            key={name}
            href={`#${name.toLowerCase()}`}
            className="nav-item bg-white text-[#FF1493] px-3 py-2 rounded-md transition-all duration-300 hover:bg-[#FF69B4] hover:text-white border-2 border-[#FF1493] hover:border-[#FF1493] hover:shadow-[0_0_10px_#FF69B4] transform hover:scale-105 flex items-center space-x-1 mb-2 md:mb-0"
            style={{ fontFamily: '"Press Start 2P", cursive' }}
