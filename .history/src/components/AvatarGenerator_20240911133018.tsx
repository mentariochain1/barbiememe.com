...
function AvatarGenerator() {
            ...
            switch (action) {
              case 'move':
                return { 
                  ...item, 
                  position: { 
                    x: Math.max(0, Math.min(x / 0.466667 - 28, 600 - 56)),
                    y: Math.max(0, Math.min(y / 0.466667 - 28, 600 - 56))
                  } 
                };
              case 'rotate':
                const center = { 
                  x: item.position.x * 0.466667 + 28, 
                  y: item.position.y * 0.466667 + 28 
                };
                const angle = Math.atan2(y - center.y, x - center.x) * (180 / Math.PI);
                return { ...item, rotation: angle };
              case 'scale':
                const dx = x - (item.position.x * 0.466667 + 28);
                const dy = y - (item.position.y * 0.466667 + 28);
                const distance = Math.sqrt(dx * dx + dy * dy);
                const maxScale = item.category === 'Hats' ? 4 : 2;
                const newScale = Math.max(0.5, Math.min(maxScale, distance / 23.33));
                return { ...item, scale: newScale };
              default:
                return item;
            }
  ...
  const handleTouchStart = (e: React.TouchEvent, id: string, newAction: 'move' | 'rotate' | 'scale') => {
    e.stopPropagation();
    setActiveAccessory(id);
    setAction(newAction);
    setIsDragging(true);
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-pink-400 to-purple-600 overflow-hidden">
      {/* Main Avatar Display */}
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <div 
          ref={avatarRef}
          className="relative w-[280px] h-[280px] bg-white rounded-lg shadow-lg overflow-hidden"
          ...
          {selectedAccessories.map((accessory) => (
            <div 
              ...
              style={{
                width: '56px',
                height: '56px',
                left: `${accessory.position.x * 0.466667}px`,
                top: `${accessory.position.y * 0.466667}px`,
                transform: `rotate(${accessory.rotation}deg) scale(${accessory.scale * 0.466667})`,
                zIndex: activeAccessory === accessory.id ? 10 : 1,
                userSelect: 'none',
              }}
              onTouchStart={(e) => handleTouchStart(e, accessory.id, 'move')}
            >
              <Image 
                src={accessory.image}
                alt={accessory.name}
              ...
              {activeAccessory === accessory.id && (
                <>
                  <div className="absolute -top-1 -left-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleTouchStart(e, accessory.id, 'scale')} />
                  <div className="absolute -top-1 -right-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleTouchStart(e, accessory.id, 'scale')} />
                  <div className="absolute -bottom-1 -left-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleTouchStart(e, accessory.id, 'scale')} />
                  <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleTouchStart(e, accessory.id, 'scale')} />
                  <div className="absolute top-1/2 -right-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleTouchStart(e, accessory.id, 'rotate')} />
                </>
              )}
            </div>
          ))}
