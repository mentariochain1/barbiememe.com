...
// @ts-expect-error - html2canvas не имеет корректных типов
...
function AvatarGenerator() {
  ...
  const captureAvatar = async () => {
    if (avatarRef.current) {
      setIsCapturing(true);
      try {
        // Временно удаляем розовую обводку со всех элементов
        const activeElements = avatarRef.current.querySelectorAll('.outline-pink-500');
        activeElements.forEach(el => {
          el.classList.remove('outline', 'outline-2', 'outline-pink-500');
        });
        ...
        activeElements.forEach(el => {
          el.classList.add('outline', 'outline-2', 'outline-pink-500');
        });
  ...
  const handleUpdateAvatar = async () => {
    if (editingAvatarIndex !== null) {
      ...
      if (imageDataUrl) {
        ...
        setError("Failed to update avatar image. Please try again.");
      }
    }
  };
