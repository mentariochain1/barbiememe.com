...
function AvatarGenerator() {
          ...
          if (item.id === activeAccessory) {
            switch (action) {
              ...
              case 'scale':
                ...
                // Увеличиваем максимальный масштаб для апок
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-pink-400 to-purple-600 overflow-hidden">
      {/* Main Avatar Display */}
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div 
          ref={avatarRef}
          className="relative w-[600px] h-[600px] bg-white rounded-lg shadow-lg overflow-hidden"
        ...
        <div className="mt-8 flex space-x-4">
          {editingAvatarIndex !== null ? (
            <button 
              ...
              className="px-8 py-4 bg-purple-500 text-white font-bold text-lg rounded-md transition-all duration-300 hover:scale-105 hover:brightness-110 animate-pulse"
              style={buttonStyle}
            >
              Update Avatar
            </button>
          ...
          <button 
            onClick={handleMint} 
            className="px-8 py-4 bg-purple-500 text-white font-bold text-lg rounded-md transition-all duration-300 hover:scale-105 hover:brightness-110 animate-float"
            style={buttonStyle}
          >
            Mint NFT
          </button>
        ...
        {error && (
          <div className="mt-4 p-2 bg-red-100 border border-red-400 text-red-700 rounded animate-shake">
            {error}
          </div>
        )}
      </div>
      ...
      <div className="w-1/3 bg-gradient-to-b from-pink-300 to-purple-400 p-6 overflow-y-auto">
        <h2 className="text-3xl font-bold mb-6 text-white sega-barbie-title">Customize Your Barbie</h2>
        ...
        <div className="flex mb-6 space-x-2 overflow-x-auto">
          {categories.map(category => (
            <button
              ...
              className={`px-4 py-2 rounded-full ${activeCategory === category ? 'bg-purple-700 text-white' : 'bg-pink-200 text-purple-700'} transition-all duration-300 hover:scale-105`}
        ...
        <div className="grid grid-cols-3 gap-4">
          {accessories
            ...
            .map((accessory) => (
              <div 
                key={accessory.id} 
                className={`p-2 border-4 border-purple-700 rounded-lg cursor-pointer transition-all duration-300 ${selectedAccessories.some(item => item.id === accessory.id) ? 'bg-pink-300 scale-105 animate-pulse' : 'bg-white hover:shadow-md hover:scale-105'}`}
              ...
              >
                <Image 
                  ...
                  className="pixelated"
                />
                <p className="text-center text-sm font-bold text-purple-700 pixel-font">{accessory.name}</p>
              </div>
            ))}
        </div>
        ...
        {/* Saved Avatars */}
        <div className="mt-8">
          <h3 className="text-2xl font-bold mb-4 text-white sega-barbie-title">Saved Avatars</h3>
          <div className="grid grid-cols-3 gap-4">
            {savedAvatars.map((avatar, index) => (
              <div 
                key={index} 
                className="relative w-full aspect-square bg-white rounded-lg overflow-hidden cursor-pointer hover:scale-105 transition-transform duration-300 shadow-neon-pink"
