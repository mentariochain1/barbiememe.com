...
function AvatarGenerator() {
  ...
  const handleActiveAccessoryToggle = (id: string) => {
    if (!isDragging) {
      setActiveAccessory(prev => prev === id ? null : id);
      setAction(null);
    }
  };
        ...
        >
          ...
          {selectedAccessories.map((accessory) => (
            <div 
              ...
              onClick={() => handleActiveAccessoryToggle(accessory.id)}
            >
              <img 
                src={accessory.image}
                alt={accessory.name}
                  ...
                  {avatar.map(accessory => (
                    <img 
                      ...
                      style={{
                        width: '40px',
                        height: '40px',
                        left: `${(accessory.position.x / 500) * 100}%`,
                        top: `${(accessory.position.y / 500) * 100}%`,
                        transform: `rotate(${accessory.rotation}deg) scale(${accessory.scale * 0.4})`,
                      }}
                      draggable="false"
                    />
                  ))}
