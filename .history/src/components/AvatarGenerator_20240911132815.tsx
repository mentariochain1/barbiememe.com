    ...
    if (selectedAccessories.length > 0) {
      ...
      if (imageDataUrl) {
        try {
          ...
          setError(null); // Очищаем ошибку, если сохранение прошло усп��шно
        } catch (error) {
          ...
          {selectedAccessories.map((accessory) => (
            ...
            >
              ...
              {activeAccessory === accessory.id && (
                <>
                  <div className="absolute -top-1 -left-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleMouseDown(e as unknown as React.MouseEvent, accessory.id, 'scale')} />
                  <div className="absolute -top-1 -right-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleMouseDown(e as unknown as React.MouseEvent, accessory.id, 'scale')} />
                  <div className="absolute -bottom-1 -left-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleMouseDown(e as unknown as React.MouseEvent, accessory.id, 'scale')} />
                  <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleMouseDown(e as unknown as React.MouseEvent, accessory.id, 'scale')} />
                  <div className="absolute top-1/2 -right-1 w-3 h-3 bg-pink-500 rounded-full" onTouchStart={(e) => handleMouseDown(e as unknown as React.MouseEvent, accessory.id, 'rotate')} />
                </>
              )}
            </div>
          ))}
