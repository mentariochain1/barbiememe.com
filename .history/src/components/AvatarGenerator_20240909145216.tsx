...
function AvatarGenerator() {
  ...
  const handleSaveAvatar = () => {
    if (selectedAccessories.length > 0) {
      const newAvatar = JSON.stringify(selectedAccessories);
      setSavedAvatars(prev => {
        if (!prev.some(avatar => JSON.stringify(avatar) === newAvatar)) {
          const updatedAvatars = [...prev, selectedAccessories];
  ...
  const handlePreviewAvatar = (avatar: SelectedAccessory[]) => {
    setSelectedAccessories(avatar);
  };
  ...
  const renderAccessories = (accessories: SelectedAccessory[], scale: number = 1) => (
    accessories.map((accessory) => (
      <div 
        ...
        style={{
          width: `${100 * scale}px`,
          height: `${100 * scale}px`,
          left: `${accessory.position.x * scale}px`,
          top: `${accessory.position.y * scale}px`,
          transform: `rotate(${accessory.rotation}deg) scale(${accessory.scale * scale})`,
          zIndex: 1,
        }}
      >
        <img 
    ...
    <div className="flex h-screen bg-gradient-to-br from-pink-400 to-purple-600">
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        >
          ...
          />
          {renderAccessories(selectedAccessories)}
          {activeAccessory && (
            <div 
              className="absolute"
              style={{
                width: '100px',
                height: '100px',
                left: `${selectedAccessories.find(a => a.id === activeAccessory)?.position.x}px`,
                top: `${selectedAccessories.find(a => a.id === activeAccessory)?.position.y}px`,
                transform: `rotate(${selectedAccessories.find(a => a.id === activeAccessory)?.rotation}deg) scale(${selectedAccessories.find(a => a.id === activeAccessory)?.scale})`,
                zIndex: 10,
              }}
            >
              <div className="absolute -top-2 -left-2 w-4 h-4 bg-pink-500 rounded-full cursor-nwse-resize" onMouseDown={(e) => handleMouseDown(e, activeAccessory, 'scale')} />
              <div className="absolute -top-2 -right-2 w-4 h-4 bg-pink-500 rounded-full cursor-nesw-resize" onMouseDown={(e) => handleMouseDown(e, activeAccessory, 'scale')} />
              <div className="absolute -bottom-2 -left-2 w-4 h-4 bg-pink-500 rounded-full cursor-nesw-resize" onMouseDown={(e) => handleMouseDown(e, activeAccessory, 'scale')} />
              <div className="absolute -bottom-2 -right-2 w-4 h-4 bg-pink-500 rounded-full cursor-nwse-resize" onMouseDown={(e) => handleMouseDown(e, activeAccessory, 'scale')} />
              <div className="absolute top-1/2 -right-2 w-4 h-4 bg-pink-500 rounded-full cursor-e-resize" onMouseDown={(e) => handleMouseDown(e, activeAccessory, 'rotate')} />
            </div>
          )}
        </div>
